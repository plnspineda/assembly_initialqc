...
group_func <- function(chrom_list) {
  df <- minimap_paf_mutate %>%
    filter(ref_name == {{chrom_list}}) %>% filter(proportion_match >= 0.1, query_align >= 1000000, proportion_query >= 0.1) %>%
    group_by(query_name) %>% summarise(orientation = names(sort(table(orientation), decreasing = TRUE)[1]), min_ref_start = min(ref_start), max_ref_end = max(ref_end),
